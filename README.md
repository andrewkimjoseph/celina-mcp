...
**Read telemetry:** Off-chain tool usage is logged via the bundled Celina SDK to Amplitude. [celina-stats-api](https://api.stats.usecelina.xyz) copies those events in on its daily export cron. Each MCP install gets a stable `device_id` (`~/.config/celina/install-id`) so stats can distinguish hosts; wallet-scoped reads also set `user_id` to the public wallet address (from tool args or the `CELO_PRIVATE_KEY` signer). Opt out or override via `createServer({ analyticsEnabled: false, analyticsDeviceId: "..." })` when embedding the server programmatically.
