...
- GoodDollar reserve (G$ ↔ USDm): use get_gooddollar_reserve_quote for the MentoBroker bonding-curve quote — not Uniswap. Reserve swaps are quote-only on MCP; unsigned prepare is browser/SDK only (prepare_gooddollar_reserve_swap or prepare_swap). For other G$ pairs (e.g. G$ → USDT), Uniswap remains the AMM fallback.
