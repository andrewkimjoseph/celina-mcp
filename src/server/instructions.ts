...
- GoodDollar wallet-link: get_gooddollar_face_verification_link (first-time verify this signer wallet), execute_connect_gooddollar_identity (link a secondary wallet to an existing verified root), execute_disconnect_gooddollar_identity. Connect requires CELO_PRIVATE_KEY to be the whitelisted GoodDollar root; pass the wallet to link as connected_account. If already verified on wallet A and MCP signer is wallet B, do not face-verify B — switch CELO_PRIVATE_KEY to wallet A and connect B.
